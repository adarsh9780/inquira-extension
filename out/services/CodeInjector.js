...
class CodeInjector {
    ...
    async insertIntoJupyterNotebook(editor, code) {
        // For Jupyter notebooks, we need to use the notebook API
        const notebook = vscode.workspace.notebookDocuments.find(doc => doc.uri.toString() === editor.document.uri.toString());
        if (!notebook) {
